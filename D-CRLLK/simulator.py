...
class Simulator(gym.Env):
    ...
    def reset(self, segment: bool = False):
        ...
        for obj in self.objects:
            ...
            if obj.optional and self.domain_rand:
                ...
                # 221111: No longer support. Change to integers.
                try:
                    obj.visible = self.np_random.randint(0, 2) == 0
                except:
                    try:
        ...
        if self.start_pose is not None:
            ...
            for _ in range(MAX_SPAWN_ATTEMPTS):
                ...
                # xinwei: fixed angle, face forward.
                if self.np_random.uniform(0, 1) < 0.5:
                    propose_angle = self.np_random.uniform(0, 0.3 * math.pi)
                else:
                    propose_angle = self.np_random.uniform(1.7 * math.pi, 2 * math.pi)
        ...
        # logger.info(f"Starting at {self.cur_pos} {self.cur_angle}") # xinwei
    ...
    # xinwei: line distance
    def get_lane_pos1(self, pos, angle):
        """
        Get the position of the agent relative to the center of the right lane
    ...
    def get_agent_info(self) -> dict:
        ...
        # xinwei
        if True:    # self.full_transparency
            #             info['desc'] = """
            #
        ...
        # xinwei: env info. position, angle.
        # info["cur_pos"] = [float(pos[0]), float(pos[1]), float(pos[2])]
        # info["cur_angle"] = float((90 - np.abs(np.rad2deg(self.cur_angle))) / 90)
        # info["robot_speed"] = self.speed
    ...
    def compute_reward(self, pos, angle, speed):
        ...
        try:
            ...
            # Compute the reward
            # reward = +1.0 * self.speed * lp.dot_dir + -10 * np.abs(lp.dist) + 40 * col_penalty
            if np.linalg.norm(self.delta_pos):
                delta_pos = self.delta_pos / np.linalg.norm(self.delta_pos)
                pose_angle = np.array([math.cos(self.cur_angle), 0, -math.sin(self.cur_angle)])
                steer_dot = np.dot(pose_angle, delta_pos)
            else:
                steer_dot = 0
            reward = +1.0 * steer_dot * self.speed * lp.dot_dir + -10 * np.abs(lp.dist) + 40 * col_penalty
            # print('speed', speed,
            #       'lp.dot_dir', lp.dot_dir,
            #       'lp.dist', lp.dist,
            #       'col_penalty', col_penalty)
            # print('reward', reward,
            #       'reward1', +1.0 * self.speed * lp.dot_dir,
            #       'reward2', -10 * np.abs(lp.dist),
            #       'reward3', 40 * col_penalty)
            # print('reward',reward,'speed',speed,'lp.dot_dir',lp.dot_dir,'lp.dist',lp.dist,'col_penalty',col_penalty)
            # xinwei
            # reward left only
            # LanePosition(dist=signedDist, dot_dir=dotDir, angle_deg=angle_deg, angle_rad=angle_rad)
            # ----------
            # signedDist: Right of the curve is negative, left is positive
            # dotDir    : Compute the alignment of the agent direction with the curve tangent
            # angle_deg :
            # angle_rad : Right of the tangent is negative, left is positive
            # face_angle = ((90 - np.abs(np.rad2deg(self.cur_angle))) / 90)
            # if face_angle < 0:
            #     reward = face_angle + 4 * col_penalty
            # else:
            #     reward = face_angle * (1 + self.speed) + 4 * col_penalty

        # # xinwei: reward structure
        # face_angle = (90 - np.abs(np.rad2deg(self.cur_angle))) / 90
        # if face_angle < 0:
        #     reward = face_angle * 0.1
        # else:
        #     reward = face_angle * 0.1 + self.speed / 0.6 * 0.00 + face_angle * self.speed / 0.6 * 1
        # # reward = reward * 0.5
