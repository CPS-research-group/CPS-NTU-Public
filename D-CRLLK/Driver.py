...
def write_to_board(update_result, recorder, curr_episode, rcpo):
    ...
    for batch_index in range(len(rcpo.driver_memory.batch_buffer)):
        total_return = sum(rcpo.driver_memory.batch_buffer[batch_index]['rewards'])
        avg_return = total_return/len(rcpo.driver_memory.batch_buffer[batch_index]['rewards'])
    ...
    if int(recorder['bool_learn']) !=  0:
        ...
        # Recorder
        'Env/total_step': recorder['total_step']/recorder['bool_learn'],
        'Env/total_reward': recorder['total_reward']/recorder['bool_learn'],
        'Env/total_done': recorder['total_done']/recorder['bool_learn'],
        # 'Env/bool_learn': recorder['bool_learn']/recorder['bool_learn'],
        'Env/total_distance': recorder['total_distance']/recorder['bool_learn'],
        'Env/total_angle': recorder['total_angle']/recorder['bool_learn'],
        'Env/robot_speed': recorder['robot_speed']/recorder['bool_learn'],
        'Env/total_lane': recorder['total_lane']/recorder['bool_learn'],
...
def main():
    ...
    rcpo = LagrangeMethod(LAGRANGE_TARGET_LANE, LAGRANGE_TARGET_COLL)
    ...
    for i, meta_agent in enumerate(meta_agents):
        job_list.append(meta_agent.job.remote(curr_episode, weights,rcpo)) #ADD RCPO HERE
        curr_episode += 1
