...
def get_roc(data: List[pandas.DataFrame],
    ...
    fpr, tpr, _ = sklearn.metrics.roc_curve(gt_all, csum_all)
...
def optimize_decay(partition_data: List[pandas.DataFrame]) -> Tuple[float]:
    ...
    """
    decay_sweep = numpy.linspace(1, 100, 100)
    top_ten = [([], [], 0, 0)] * 10
    for decay in list(decay_sweep):
        fpr, tpr, auroc = get_roc(partition_data, decay)
...
if __name__ == '__main__':
    ...
    cal_data['PARAMS']['decay'][args.partition] = opt_decay
