...
import torch
import torchvision
from bvae import Encoder, BetaVae
...
class BetaVAEDetector:
    ...
    def __init__(self,
        ...
        with open(alpha_cal, 'r') as alpha_cal_f:
            ...
            for idx, item in enumerate(cal_data.items()):
                ...
                #self.z_mask[idx, item[1]['top_z'][1][0]] = 1
        ...
        self.encoder = torch.load(weights)
    ...
    def run_detect(self, video: str) -> None:
        ...
        ood_part = ''
        match = re.findall('_([a-zA-Z]+)(-?\d+\.\d+)', video)
        if match:
            for m in match:
    ...
    def inference(self, frame: torch.Tensor) -> numpy.ndarray:
        ...
        """
        with torch.no_grad():
            _, mu, logvar = self.encoder(frame.unsqueeze(0).to(self.device))
