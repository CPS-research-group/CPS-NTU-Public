...
def process_folder(folder: str,
    ...
    new_dir_name = f"{folder}_rain{rain_level}_brightness{brightness_level}_" \
                   f"snow{snow_level}_m{mode}"
        ...
        if count >= ood_start and count <= ood_stop:
            if rain_level > 0.0:
                if mode == 4:
                    rain_level = random.uniform(min_level, orig_rain)
                if mode == 5:
                    rain_level = ((rain_level + orig_rain / 3) % orig_rain) \
                        + min_level
                rain_mask = update_mask(rain_mask, rain_level, rain_speed)
            ...
            if brightness_level != 0.0:
                if mode == 4:
                    brightness_level = random.uniform(
                        ...
                        orig_brightness)
                if mode == 5:
                    brightness_level = ((brightness_level + orig_brightness \ 
                        / 3) % orig_brightness) + min_level
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='1: whole video or folder becomes OOD; 2: 1st half of video or '
             ...
             'intensity from min_level to max_level for entire video.')
