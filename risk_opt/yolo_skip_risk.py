...
have_ood = 0
for i in ID_RANGE:
    fp_count_id += pd.read_csv(f'yolo_skip_frame/fp_count_{i}.csv').to_numpy()[:, 1:]
    empty_id += pd.read_csv(f'yolo_skip_frame/empty_count_{i}.csv').to_numpy()[:, 1:]
    fn_count_id += pd.read_csv(f'yolo_skip_frame/fn_count_{i}.csv').to_numpy()[:, 1:]
    have_id += pd.read_csv(f'yolo_skip_frame/have_count_{i}.csv').to_numpy()[:, 1:]
for j in OOD_RANGE:
    fp_count_ood += pd.read_csv(f'yolo_skip_frame/fp_count_{j}.csv').to_numpy()[:, 1:]
    empty_ood += pd.read_csv(f'yolo_skip_frame/empty_count_{j}.csv').to_numpy()[:, 1:]
    fn_count_ood += pd.read_csv(f'yolo_skip_frame/fn_count_{j}.csv').to_numpy()[:, 1:]
    have_ood += pd.read_csv(f'yolo_skip_frame/have_count_{j}.csv').to_numpy()[:, 1:]
...
for IDX, TD in enumerate(TDS):
    ...
    for i in range(len(OOD_SIZES)):
        for j in range(14):
            p_miss_ood[i, j] = np.count_nonzero(total_t[i,j, :] > TD) / SAMPLES
            p_miss_yolo[i, j] = np.count_nonzero(t_yolo[j, :] > 2 * TD) / SAMPLES
    PE = np.zeros(SPACE)
    ...
    #RISK[U >= f[TD]] = np.inf
    np.save(f'risk_{TD}_skip_yolo_first.npy', RISK)
    np.save(f'U_{TD}_skip_yolo_first.npy', U)
    ...
    levels = ax[0][IDX].contourf(X0, Y0, RISK[:,:,min_risk[2],min_risk[3]], extent=[8,224,0,1], vmin=0, vmax=1)
    #fig.colorbar(levels)
    ax[0][IDX].set_xlabel('OOD input size')
    ax[0][IDX].set_ylabel('OOD threshold')
    ax[0][IDX].set_title(f'Risk')

    ax[1][IDX].contourf(X, Y, RISK[min_risk[0], min_risk[1], :,:], extent=[64, 480, 0, 1], vmin=0, vmax=1)
    ax[1][IDX].set_xlabel('YOLO input size')
    ax[1][IDX].set_ylabel('YOLO threshold')
    ax[1][IDX].set_title('Risk')

    ax[2][IDX].contourf(X2, Y2, np.transpose(PE[min_risk[0], :, min_risk[2], :]),extent=[8, 224, 64, 480], vmin=0, vmax=1)
    ax[2][IDX].set_xlabel('OOD input size')
    ax[2][IDX].set_ylabel('YOLO input size')
    ax[2][IDX].set_title(f'P(E)')

    ax[3][IDX].contourf(X2, Y2, np.transpose(PEPSILON[min_risk[0], :, min_risk[2], :]),extent=[8, 224, 64, 480], vmin=0, vmax=1)
    ax[3][IDX].set_xlabel('OOD input size')
    ax[3][IDX].set_ylabel('YOLO input size')
    ax[3][IDX].set_title(f'P(EPSILON)')
